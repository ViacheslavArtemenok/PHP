...
session_start();
include "config.php";
setcookie('arr_goods', getGoods($connect));
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    function getGoods($connect)
    {
        if (!empty($_POST['good_name'])) { //ДОБАВЛЯЕМ ТОВАР В КУКИ
            ...
            if ($change == 0) {
                $temp_arr[] =  ['good_id' => $_POST['good_id'], 'gender' => $_POST['good_number_g'], 'good' => $_POST['good_name'], 'size' => $_POST['good_size'], 'price' => $_POST['good_price'], 'image' => $_POST['good_photo_name'], 'quantity' => $_POST['good_quantity']];
            }
            ...
            return '';
        } elseif (!empty($_GET['pay'])) { //ОФОРМЛЕНИЕ ЗАКАЗА С ОТПРАВКОЙ В БД И ФАЙЛ JSON
            $temp_arr = json_decode($_COOKIE['arr_goods'], true);
